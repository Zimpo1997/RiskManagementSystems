                ...
                <div class="col-md-6 col-sm-12 ">
                    <nav aria-label="breadcrumb" role="navigation" class="float-right">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">แก้ไขบทบาท</li>
                        </ol>
                    </nav>
                </div>
        ...
        <div class="pd-20 card-box mb-30">
            <div class="mb-5">
                <div class="text-center">
                    <h4 class="text-blue h4">แก้ไขบทบาท</h4>
                </div>
            </div>
            <form action="/roles/edit/{{ $roles->id }}" method="POST">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select class="selectpicker form-control" id="permission" name="permission[]" data-size="5" multiple
                        ...
                        @foreach ($permissions as $permission)
                            <option value="{{ $permission->id }}" @foreach ($mypermissions as $mypermission) {{ $permission->id == $mypermission->id ? 'selected' : '' }} @endforeach>{{ $permission->name_th }}</option>
                        @endforeach
                    </select>
                </div>
