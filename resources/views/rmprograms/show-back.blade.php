...
@section('contents')
                ...
                <div class="col-md-6 col-sm-12 ">
                    <nav aria-label="breadcrumb" role="navigation" class="float-right">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">รายการความเสี่ยงส่งคืน</li>
                        </ol>
                    </nav>
                </div>
                        ...
                        <tr id="trID_{{ $risk->id }}">
                            ...
                            <td class="text-center">
                                ...
                                @can('rm_program_back_risk')
                                    <button type="button" class="btn btn-sm mb-1 showModal" title="ส่งคืนแอดมิน"
                                        data-bgcolor="#db4437" data-color="#ffffff" data-toggle="modal"
                                        data-target="#modalBackRisk" onclick="getIdRisk({{ $risk->id }})">
                                        <i class="dw dw-curved-arrow1"></i>
                                    </button>
                                @endcan
                                @can('rm_program_send_risk')
                                    <button type="button" class="btn btn-sm btn-primary mb-1 showModal" title="ส่งต่อหน่วยงาน"
                                        data-toggle="modal" data-target="#exampleModalCenter" onclick="getIdRisk({{ $risk->id }})">
    ...
    <div class="modal fade bd-example-modal-lg" id="exampleModalCenter" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <form id="frm-sendagainagency" method="post">
                    @csrf
                    <div class="modal-header">
...
@section('scripts')
    ...
    <script>
        function getIdRisk(id) {
            ...
            var _token = $('meta[name="csrf-token"]').attr('content');
                ...
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: '/risks/rmprogram/check',
                    data: {
                        'risk_id': id,
                        _token: _token
                    },
                    success: function(data) {
                        var workgroup = data.mainrisks.respon_workgroup.id;
                        var agencies_join = [];
                        $.each(data.mainrisks.agencies_join, function(i, v) {
                            agencies_join.push(v.id);
                        });
                        var teamrisks_join = [];
                        $.each(data.mainrisks.teamrisks_join, function(i, v) {
                            teamrisks_join.push(v.id);
                        });

                        $("#respon_workgroup").select2("val", [workgroup]);
                        $('#mainrisks_agencies_join').val(agencies_join).trigger('change');
                        $('#mainrisks_teamrisks_join').val(teamrisks_join).trigger('change');
                    }
                });
        }
        ...
        $(function() {
            ...
            $("#frm-sendagainagency").submit(function(e) {
                e.preventDefault();
                        ...
                        .then((willDelete) => {
                            if (willDelete) {
                                $.ajax({
                                    ...
                                    data: $("#frm-sendagainagency").serializeArray(),
                                    success: function(data) {
